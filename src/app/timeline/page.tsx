  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="hv-panel p-6">
        <ol className="relative ml-3 space-y-6 border-l border-cyan-100/20 pl-6">
          {EVENTS.map((event) => (
            <li key={event.date + event.title}>
              <span className="absolute -left-1.5 mt-1.5 h-3 w-3 rounded-full border-2 border-cyan-400 bg-cyan-400/20" />
              <time className="font-mono text-xs uppercase tracking-wider text-cyan-50/58">
                {event.date}
              </time>
              <p className="mt-1 text-base text-cyan-50/78">{event.title}</p>
            </li>
          ))}
        </ol>
      </div>
